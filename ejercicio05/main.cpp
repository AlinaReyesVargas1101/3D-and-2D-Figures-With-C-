...
void drawFloor() {
    GLint Lado = 100;
    glColor4ub(255, 0, 0, 0); //line color green
    //glTranslatef(0, 0, 80); //x, y, z
    glRotatef(180, 0, 1, 0);
    glBegin(GL_QUADS);
        glVertex3f(-Lado, 89, -Lado); //Punto 1
        glVertex3f(Lado, 89, -Lado); //Punto 2
        glVertex3f(Lado, 89, Lado); //Punto 3
        glVertex3f(-Lado, 89, Lado); //Punto 4
    glEnd();
}
void drawCopa(void) {
   glPushMatrix();
...
void draw_scene(void)
{
   ...
   glScalef(Zoom, Zoom, Zoom); //Escala toda la escena
    ...
    glPopMatrix();
    glPushMatrix();
        glTranslatef(0, -128, 0); //x, y, z
        drawFloor();
    glPopMatrix();
    glPushMatrix();
        glTranslatef(75,-17, 65); //x, y, z
        drawHelicopter();
    glPopMatrix();
...
int main(int argc, char** argv)
{
   ...
   glutInitWindowSize (800, 800); //Tamaño de la ventana 500 x 500
   glutInitWindowPosition (100, 100); //Posición de la ventana en la pantalla
   glutCreateWindow ("Trompo 1.0"); //Crea la ventana y le pone la etiqueta
